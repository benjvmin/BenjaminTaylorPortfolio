  ...
  methods: {
      ...
      fetch(`${this.$hostname.name}Posts/?slug=${this.$route.params.title}`)
        ...
        .then(json => {
          ...
          this.lazyLoadSyntaxHighlighting();
				...
				.then(() => { this.storeDataForSocialLinks(); })
				.then(() => { this.$eventBus.$emit('storeSocialLinkData'); })
        .catch(err => {
          this.error = true;
          console.log(err);
        });
    ...
    lazyLoadSyntaxHighlighting() {
      ...
      document.head.appendChild(script);
		...
		storeDataForSocialLinks() {
			window.sessionStorage.setItem('post-title', this.post.title.rendered);
			window.sessionStorage.setItem('post-description', document.querySelector('.post-content').textContent.substring(0, 100));
		}
  },
  created() {
		this.fetchPost();
		// this.storeDataForSocialLinks();
  }
};
</script>
